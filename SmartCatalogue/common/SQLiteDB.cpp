...
using namespace std;
string SQLiteDB::returnData;
bool SQLiteDB::gettingData;
/*
...
//--------------------------------------------------------------------------------------------------
 bool SQLiteDB::openSQLiteDB(string name, string &output)
 {
	 // Open SQLiteDB
	SQLiteDBName = name;
   ...
   if (returnCode)
   {
	  ...
	  output += ("Error opening SQLite3 SQLiteDB: " +errMsg + "\n");
      sqlite3_close(db);
	  return false;
   }
...
//--------------------------------------------------------------------------------------------------
 void SQLiteDB::closeSQLiteDB(string &output)
 {
	  // Close SQLiteDB if its open
	if (db == NULL)
		 return;
   output = ("Attepmting to Close " + SQLiteDBName + "...");
   sqlite3_close(db);
   output +=  "Closed";
   lastError.clear();
 }
...
//--------------------------------------------------------------------------------------------------
void SQLiteDB::setTableName(std::string name)
{
	curTableName = name;
}
//--------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
bool SQLiteDB::executeSQL(string command, string &output)
{
	char *error;
	returnData = "";
...
//--------------------------------------------------------------------------------------------------
bool SQLiteDB::createTable(string name, string query)
{
	setTableName(name);
	string output;
...
//--------------------------------------------------------------------------------------------------
bool SQLiteDB::insertData(string query) 
{
...
//--------------------------------------------------------------------------------------------------
vector<string> SQLiteDB::getAllTables() 
{
...
//--------------------------------------------------------------------------------------------------
vector<string> SQLiteDB::viewData() 
{
...
//--------------------------------------------------------------------------------------------------
vector<string> SQLiteDB::query(char* query)
{
...
//--------------------------------------------------------------------------------------------------
vector<vector<string>> SQLiteDB::queryV2(char* query)
{
	sqlite3_stmt *statement;
